...
export const DashboardFresh: React.FC<DashboardFreshProps> = (props) => {
  ...
  const renderMode = () => {
    ...
    if (mode === 'automation') return <AutomationSequences emailCadences={emailCadences} followUpTasks={followUpTasks} leads={leads} onCompleteTask={props.onCompleteTask} onSelectLeadById={(id) => { const lead = leads.find((item) => item.id === id); if (lead) props.onSelectLead(lead); }} onLaunchDialerForLead={props.onLaunchDialerForLead} />;
    ...
    return (
      <>
        <section className="fresh-metrics">
          <div className="fresh-metric"><span><Users size={16} /> Active leads</span><strong>{leads.length}</strong><small><b>+14%</b> from last week</small></div>
        ...
        <section className="fresh-grid">
          <div className="fresh-panel action-panel">
            ...
            <div className="fresh-signal"><span className="signal-dot" /> Engine ready <span>Google Maps + social signals</span><b>18ms</b></div>
          </div>
        ...
        <section className="fresh-lower-grid">
          ...
          <div className="fresh-panel channel-panel"><div className="fresh-panel-heading compact-heading"><div><span className="fresh-kicker">Acquisition mix</span><h2>Where leads come from.</h2></div></div>{[['Google Maps', 'google_maps', '#0F1C2E'], ['Meta Ads', 'meta_ads', '#3d5d7d'], ['Social scraper', 'social_scrape', '#168454'], ['Web scrape', 'web_scrape', '#5b8bb8']].map(([label, source, color]) => { const count = leads.filter((lead) => lead.sourceChannel === source).length; const width = `${leads.length ? Math.max((count / leads.length) * 100, count ? 9 : 0) : 0}%`; return <div className="fresh-channel" key={source}><div><span>{label}</span><b>{count}</b></div><div className="fresh-channel-track"><i style={{ width, background: color }} /></div></div>; })}<div className="fresh-channel-footer"><span><span className="status-dot" /> All systems operational</span><button className="fresh-text-button" onClick={() => setMode('campaigns')}>Manage sources <ArrowRight size={14} /></button></div></div>
        </section>
      </>
    );
  };

  return <div className="fresh-dashboard"><div className="fresh-dashboard-top"><div><span className="fresh-kicker">Workspace / Command center</span><h1>Good morning, Alex.</h1><p>Here’s what is moving across your pipeline today.</p></div></div>{renderMode()}</div>;
};
